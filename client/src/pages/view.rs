    ...
    view! {
        <>
            <div class="nav">
                ...
                <div class="controls">
                    {move || {
                        // RESOLVED: Merge owner check from main with security params from feat/keys
                        let is_owner = project_data.get()
                            .and_then(|data| data.and_then(|d| d.get("owner_id").and_then(|id| id.as_i64())))
                            .zip(user.get())
                            .map_or(false, |(owner_id, u)| owner_id == u.id);
                                    ...
                                    view! {
                                        <button class="btn-primary btn-success" 
                                                ...
                                                on:click=move |_| {
                                                    let u_val = username();
                                                    let s_val = slug();
                                                    // Extract security fields added in feat/keys
                                                    ...
                                                    // Call update function signature
